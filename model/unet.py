...
#         self.decode_layer3 = DecodeLayer(generator_dim*8*2, generator_dim*8, inst_norm, embedding_num, norm_type=norm_type)
        self.decode_layer4 = DecodeLayer(generator_dim*8*2, generator_dim*4, inst_norm, embedding_num, norm_type=norm_type)
        self.decode_layer5 = DecodeLayer(generator_dim*4*2, generator_dim*2, inst_norm, embedding_num, norm_type=norm_type)
        self.decode_layer6 = DecodeLayer(generator_dim*2*2, generator_dim, inst_norm, embedding_num, norm_type=norm_type)
        self.decode_layer7 = DecodeLayer(generator_dim*2, output_dim, inst_norm, embedding_num, do_concat=False, norm_type=norm_type)
        self.attn = Self_Attn(generator_dim * 2, 'relu')

    def forward(self, encoded, encoding_layers, ids, one_hot_ids):
        d1 = self.decode_layer1(encoded, False, encoding_layers["e6"], ids, one_hot_ids)
        d2 = self.decode_layer2(d1, False, encoding_layers["e5"], ids, one_hot_ids)
        d3 = self.decode_layer3(d2, False, encoding_layers["e4"], ids, one_hot_ids)
#         d4 = self.decode_layer4(d3, False, encoding_layers["e3"], ids, one_hot_ids)
        d4 = self.decode_layer4(d3, False, encoding_layers["e3"], ids, one_hot_ids)
        d5 = self.decode_layer5(d4, False, encoding_layers["e2"], ids, one_hot_ids)
        d6 = self.decode_layer6(d5, False, encoding_layers["e1"], ids, one_hot_ids)
        d_6 = self.attn(d6)
        d7 = self.decode_layer7(d_6, True, None, ids, one_hot_ids)
...
class Discriminator(nn.Module):
    def __init__(self, input_dim, discriminator_dim, embedding_num, input_width, norm_type):
        ...
        self.attn = Self_Attn( discriminator_dim*8 , 'relu')
        ...
        self.conv3 = Conv2d(discriminator_dim*4, discriminator_dim*8)
        ...
        num_features = int(discriminator_dim*8*8*8)
    ...
    def forward(self, image, y=None):
        ...
        fc1 = self.fc1(h4.view(h4.shape[0], -1))
        # category loss
        fc2 = self.fc2(h4.view(h4.shape[0], -1))

        return torch.sigmoid(fc1), fc1, fc2

class UNet(nn.Module):
    def __init__(self,  device, experiment_dir=None, experiment_id=0, batch_size=16, input_width=128, output_width=128,
                 generator_dim=64, discriminator_dim=64, L1_penalty=100, Lconst_penalty=15, Ltv_penalty=0.0,
                 Lcategory_penalty=1.0, embedding_num=29, embedding_dim=128, input_filters=1, output_filters=1,
                 inst_norm=False, g_norm_type="bn", d_norm_type="bn", gan_loss_type="vanilla", cycle_gan=False,
                 rotate_range=0):
        # 우린 1차원 모델이니깐 input_filter = out_filter = 1
    ...
    def restore_model(self, model_dir, is_training=True):
        ...
        if os.path.exists(checkpoint_path):
            ...
            if is_training:
                ...
                if self.cycle_gan:
                    self.c_generator.load_state_dict(ckpt['c_generator'],strict=False)
                    self.c_discriminator.load_state_dict(ckpt['c_discriminator'],strict=False)
...
#         real_imgs = self.real_B.permute(0,2,3,1).cpu().numpy()
        ...
        real_imgs = self.real_B.cpu().numpy()

        merged_fake_images = merge(scale_back(fake_imgs), [self.batch_size, 1])
        merged_real_images = merge(scale_back(real_imgs), [self.batch_size, 1])
        merged_pair = np.concatenate([merged_real_images, merged_fake_images], axis=1)
#         merged_pair  = merged_pair.astype(np.uint8) # 수정
...
##############################모델 훈련시키는 부분#################################            
    def train(self, lr=0.0001, epoch=100, schedule=10, resume=True, flip_labels=False,
